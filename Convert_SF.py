...
def convert_batch(opendir, savedir, files='all'):
    if files == 'all':  # Convert all files in folder
        files = []
        for f in os.listdir(opendir):
            if f.endswith('.sfmov') or f.endswith('.sfimg'):
                files.append(f)

    files = [files] if isinstance(files, str) else files
    for x in files:
        t = Convert_sf(opendir, savedir, x)
        t.convert()
#    t = [Convert_sf(opendir, savedir, x) for x in files].convert()
